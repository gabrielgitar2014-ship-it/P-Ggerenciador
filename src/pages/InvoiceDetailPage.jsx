  ...
  useEffect(() => {
    const fetchComprovante = async () => {
      ...
      try {
        ...
        console.error('Erro ao buscar comprovante da fatura:', err);
      }
    };
    fetchComprovante();
  }, [banco, selectedMonth]);
  ...
  return (
    ...
    >
      ...
      <div className={`p-6 pb-8 rounded-b-[2rem] shadow-xl bg-gradient-to-br ${theme.gradient} text-white shrink-0`}>
         ...
         <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/10 flex flex-col gap-4">
            ...
            <div className="border-t border-white/20 pt-3 flex items-center justify-between">
              {comprovanteUrl ? (
                <div className="flex items-center gap-2">
                  ...
                  <a href={comprovanteUrl} target="_blank" rel="noopener noreferrer" className="text-sm text-white underline hover:text-white/80 truncate max-w-[120px] sm:max-w-[200px]">
                    Ver Comprovante
                  </a>
                  <label className="ml-2 text-[10px] uppercase font-bold bg-white/20 px-2 py-1 rounded cursor-pointer hover:bg-white/30 transition-colors">
                    Trocar
                    <input type="file" className="hidden" accept="image/*,application/pdf" onChange={handleUploadComprovante} disabled={uploading} />
                  </label>
                </div>
              ) : (
                <div className="flex items-center gap-2">
                  <label className="flex items-center gap-2 text-sm bg-white/20 px-3 py-1.5 rounded-lg cursor-pointer hover:bg-white/30 transition-colors">
                    {uploading ? <div className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" /> : <Upload className="w-4 h-4" />}
                    <span className="font-medium text-sm">{uploading ? 'Enviando...' : 'Anexar Comprovante'}</span>
                    <input type="file" className="hidden" accept="image/*,application/pdf" onChange={handleUploadComprovante} disabled={uploading} />
                  </label>
                </div>
              )}
