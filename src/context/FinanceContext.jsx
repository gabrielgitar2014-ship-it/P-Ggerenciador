...
export function FinanceProvider({ children }) {
  ...
  const savePaymentMethod = async (methodData) => {
    try {
      ...
      let result;
      if (methodData.id) {
        const { data, error } = await supabase.from('metodos_pagamento').update(payload).eq('id', methodData.id).select();
        if (error) throw error;
        if (!data || data.length === 0) throw new Error("Aviso: Cartão padrão do sistema ou você não tem permissão para editá-lo. Crie um novo cartão.");
        result = data[0];
      } else {
