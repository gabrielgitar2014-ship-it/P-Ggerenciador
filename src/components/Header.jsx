...
import { Settings, Eye, EyeOff, Sun, Moon, Monitor, RefreshCw, CheckCircle2, LogOut } from 'lucide-react';
...
export default function Header({ selectedMonth, onGoToHome }) {
  ...
  const { isSyncing, lastSyncedAt, syncNow, setError } = useFinance();
        ...
        <div className="relative flex-1 flex items-center justify-start">
          ...
          {menuOpen && (
             <div ref={menuRef} className="absolute left-0 top-12 w-64 rounded-xl border border-black/10 dark:border-white/10 shadow-xl bg-white dark:bg-slate-800 overflow-hidden">
               ...
               <div className="border-t border-black/10 dark:border-white/10 my-1 mx-2"></div>

               <MenuItem 
                 onClick={handleLogout} 
                 icon={LogOut} 
                 label="Sair" 
                 helper="Desconectar da sua conta" 
               />
             </div>
          )}
        </div>

        {/* Centro: Título clicável para voltar ao início */}
        <div className="flex-1 flex items-center justify-center">
          <button
            onClick={onGoToHome} // Chama a prop recebida do componente pai
            className="transition-transform transform hover:scale-105 active:scale-95 rounded-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 cursor-pointer"
            aria-label="Voltar para a tela inicial"
            title="Voltar para o início"
          >
            <h1 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-fuchsia-500 select-none">
              GenFinance
            </h1>
          </button>
        </div>
