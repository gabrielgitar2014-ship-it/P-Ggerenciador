...
// (A linha que estava aqui foi REMOVIDA)
...
export default function Header({ selectedMonth, onGoToHome }) {
  ...
  // <<< [INÍCIO DA CORREÇÃO] >>>
  // A linha foi movida para DENTRO da função Header, 
  // que é o local correto, e inclui o 'setError'.
  ...
  const handleLogout = async () => {
    setMenuOpen(false);
    setError(null); // Agora 'setError' está definido e vai funcionar
    try {
    ...
    <header className="sticky top-0 z-40 bg-white/70 dark:bg-slate-900/60 backdrop-blur border-b border-black/5 dark:border-white/5">
      <div className="flex items-center justify-between px-3 py-3 h-16">
        ...
        <div className="flex-1 flex items-center justify-center">
          <button
            onClick={onGoToHome}
            className="transition-transform transform hover:scale-105 active:scale-95 rounded-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
