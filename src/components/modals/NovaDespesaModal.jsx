...
const buildParcelas = ({ despesaId, total, n, startDateYYYYMMDD }) => {
  ...
  const start = new Date(year, month - 1, 1); // Começa sempre no dia 1 para cálculo mensal
  ...
  for (let k = 1; k <= n; k++) {
    ...
    const dd = '01'; // As parcelas são associadas ao 1º dia do mês
    parcelas.push({
...
export default function NovaDespesaModal({ onClose, despesaParaEditar }) {
  const { saveVariableExpense, categorias } = useFinance(); // Adiciona categorias
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const dadosParaSalvar = {
        ...
        device_id: deviceId,
        categoria_id: formData.categoria_id, // Inclui o ID da categoria
      };
      ...
      // O fetchData deve ser tratado pelo contexto pai ou App.jsx se for navegação de página
      ...
      <form onSubmit={handleSubmit} className="space-y-4 px-6 pb-6 max-h-[80vh] overflow-y-auto">
        ...
        <Card className="shadow-none border-0">
          ...
          <CardContent className="space-y-4 p-0">
            ...
            {/* 3. CAMPO DE CATEGORIA (NOVO E OBRIGATÓRIO) */}
            <div className="grid w-full items-center gap-1.5">
                ...
                >
                    ...
                    {/* CORREÇÃO APLICADA AQUI: Adicionado scroll na SelectContent */}
