...
export default function GeneralTab({ selectedMonth, parcelasDoMes, onNavigate }) {
  const { transactions, loading, fetchData, saveIncome } = useFinance();
  ...
  const financialSummary = useMemo(() => {
    ...
    const allExpenses = [...monthlyTransactions.filter(t => t.is_fixed && t.type === 'expense'), ...(parcelasDoMes || [])];
  ...
  const handleEditIncome = (income) => showModal('novaRenda', { incomeToEdit: income, onSave: saveIncome });
  const handleAddNewIncome = () => showModal('novaRenda', { onSave: saveIncome });
