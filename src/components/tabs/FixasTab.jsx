...
const FixedExpenseItem = ({ item, onEdit, onDelete, onTogglePay, onUploadRequest, valuesVisible }) => {
      ...
      <AnimatePresence>
        {showActions && (
          ...
          >
            ...
            {!item.comprovante_url && (
              <button 
                onClick={() => onUploadRequest(item)}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 text-xs font-bold rounded-lg hover:bg-purple-100 transition-colors"
              >
...
export default function FixasTab({ onBack, selectedMonth }) {
  ...
  const { transactions, deleteDespesa, toggleFixedExpensePaidStatus, saveFixedExpense, fetchData } = useFinance();
  ...
  const handleTogglePay = async (item) => {
    try {
      if (toggleFixedExpensePaidStatus) {
        ...
        // Se a despesa foi marcada como paga (antes era false) e não tem comprovante
        if (!item.paid && !item.comprovante_url) {
          showModal('alert', {
            title: 'Despesa Paga!',
            description: 'Deseja enviar o comprovante de pagamento agora?',
            confirmText: 'Sim',
            cancelText: 'Não',
            onConfirm: () => {
  ...
  const handleUploadRequest = (item) => {
  ...
  return (
    <>
      ...
      {/* Input oculto para upload de comprovantes */}
      <input 
        type="file" 
        ref={fileInputRef} 
