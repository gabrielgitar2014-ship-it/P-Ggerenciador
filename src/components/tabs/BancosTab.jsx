...
import { CreditCard, Wallet, ChevronRight, PieChart, Plus, X, Check, Upload, CheckCircle, Download, FileText, Eye, Edit2 } from 'lucide-react';
...
const BankCard = ({ banco, valor, onClick, index, selectedMonth, comprovanteUrl, onUploadSuccess, onEdit }) => {
  ...
  const handleUpload = async (e) => {
    e.stopPropagation(); // Evita o clique do cartão
    ...
    try {
      ...
      // Inserir ou atualizar na tabela faturas_comprovantes
      const { data: existing } = await supabase
        .from('faturas_comprovantes')
        .select('id')
        .eq('banco_nome', banco.nome)
      ...
      if (existing) {
        await supabase
          .from('faturas_comprovantes')
          .update({ comprovante_url: newUrl })
          .eq('id', existing.id);
      } else {
        await supabase
          .from('faturas_comprovantes')
          .insert({ banco_nome: banco.nome, mes: selectedMonth, comprovante_url: newUrl });
      }

      onUploadSuccess(banco.nome, newUrl);
    } catch (err) {
  ...
  const handleDownload = async (e) => {
    ...
    try {
        ...
        a.href = url;
        // Nome padrão do arquivo a ser baixado
        a.download = `comprovante_${banco.nome}_${selectedMonth}.pdf`;
  ...
  return (
    <motion.div
      ...
      className={`
        ...
        rounded-[1.5rem] p-6 h-auto min-h-[14rem] flex flex-col justify-between
    ...
    >
      ...
      <div className="flex justify-between items-start relative z-10">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center font-bold text-lg border border-white/10 shrink-0">
            {banco.nome.charAt(0)}
          </div>
          <span className="font-bold text-lg tracking-wide truncate pr-2">{banco.nome}</span>
        </div>
        
        {/* TAG DE COMPROVANTE ENVIADO */}
        {comprovanteUrl ? (
          <div className="flex items-center gap-1 bg-emerald-500/80 backdrop-blur-sm text-white px-2 py-1 rounded-full text-[10px] font-bold shadow-sm whitespace-nowrap" title="Comprovante Enviado">
      ...
      <div className="relative z-10 mt-4 flex-1">
        ...
        <div className="flex items-center justify-between mt-1">
          ...
          <div className="p-1.5 bg-white/20 rounded-full opacity-0 group-hover:opacity-100 transition-opacity shrink-0 ml-2">
             <ChevronRight className="w-5 h-5" />
          </div>
        </div>
      </div>

      {/* AÇÕES DO COMPROVANTE E INFOS */}
      <div className="relative z-10 mt-4 border-t border-white/20 pt-3 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          {comprovanteUrl ? (
            <>
               <button onClick={handleView} className="p-1.5 bg-white/20 hover:bg-white/30 rounded-lg backdrop-blur-sm transition-colors text-white" title="Visualizar">
                 <Eye className="w-4 h-4" />
               </button>
               <button onClick={handleDownload} className="p-1.5 bg-white/20 hover:bg-white/30 rounded-lg backdrop-blur-sm transition-colors text-white" title="Baixar">
                 <Download className="w-4 h-4" />
               </button>
               <label className="text-[10px] uppercase font-bold bg-white/20 hover:bg-white/30 px-2 py-1.5 rounded-lg cursor-pointer transition-colors" onClick={(e) => e.stopPropagation()}>
                  Trocar
                  <input type="file" className="hidden" accept="image/*,application/pdf" onChange={handleUpload} disabled={uploading} />
               </label>
            </>
          ) : (
            <label className="flex items-center gap-2 text-[11px] font-bold bg-white/20 hover:bg-white/30 px-2 py-1.5 rounded-lg cursor-pointer transition-colors" onClick={(e) => e.stopPropagation()}>
               {uploading ? <div className="w-3 h-3 border-2 border-white/40 border-t-white rounded-full animate-spin" /> : <Upload className="w-3 h-3" />}
          ...
          {/* BOTÃO EDITAR CARTÃO */}
          <button 
            onClick={(e) => { e.stopPropagation(); onEdit(banco); }} 
        ...
        <div className="flex items-center gap-2 text-xs font-mono opacity-60 ml-auto">
           {banco.diaFechamento && (
             <span className="text-[10px] bg-black/30 px-2 py-1 rounded text-white/80 font-sans">
               Fecha {banco.diaFechamento}
             </span>
           )}
        </div>
      </div>
...
const CardModal = ({ onClose, onSave, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setCor(initialData.cor || COLOR_OPTIONS[0].value);
      setDiaFechamento(initialData.diaFechamento || initialData.dia_fechamento || '');
      setDiaVencimento(initialData.diaVencimento || initialData.dia_vencimento || '');
      setUltimosDigitos(initialData.ultimosDigitos || initialData.ultimos_digitos || '');
    }
  }, [initialData]);
...
export default function BancosTab({ selectedMonth, onCardClick, onBack }) {
    ...
    const fetchComprovantes = async () => {
      try {
        const { data, error } = await supabase
          .from('faturas_comprovantes')
          .select('banco_nome, comprovante_url')
          .eq('mes', selectedMonth);
        if (data) {
          const map = {};
          data.forEach(d => map[d.banco_nome] = d.comprovante_url);
          setComprovantes(map);
        }
      } catch (err) {
        console.error('Erro ao buscar comprovantes das faturas:', err);
      }
    };
    if (selectedMonth) {
      fetchComprovantes();
  ...
  const handleUploadSuccess = (bancoNome, url) => {
    setComprovantes(prev => ({ ...prev, [bancoNome]: url }));
  };
  ...
  const handleSaveCard = async (cardData) => {
    ...
    if (result.ok) {
      ...
      alert(result.error?.message || "Erro ao salvar cartão.");
    }
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {dadosCartoes.map((item, index) => (
          <BankCard 
            ...
            comprovanteUrl={comprovantes[item.nome]}
