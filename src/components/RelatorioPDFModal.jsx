...
const RelatorioPDFModal = (props) => {
  ...
  useEffect(() => {
    if (despesasOriginais && despesasOriginais.length > 0) {
      // Extrai todas as datas válidas (considera date OU data_compra)
  ...
  const handleGerarPDF = () => {
    try {
      ...
      // 2. Filtra por "Período" (considera date OU data_compra)
      ...
      if (colunas.data) { 
        head[0].push('Data'); 
        colunasAtivas.push('data'); 
      }
      if (colunas.data_compra) { 
        head[0].push('Data compra'); 
        colunasAtivas.push('data_compra'); 
      }
      if (colunas.descricao) { 
        head[0].push('Descrição'); 
        colunasAtivas.push('descricao'); 
      }
      if (colunas.parcela) { 
        head[0].push('Parcela'); 
        colunasAtivas.push('parcela'); 
      }
      if (colunas.categoria) { 
        head[0].push('Categoria'); 
        colunasAtivas.push('categoria'); 
      }
      if (colunas.valor) { 
        head[0].push('Valor'); 
        colunasAtivas.push('valor'); 
      }
      ...
      despesasFiltradas.forEach(d => {
        ...
        colunasAtivas.forEach(col => {
          switch(col) {
            case 'data': {
              // Aqui consideramos a "data principal" (date),
              // caindo para data_compra se não houver.
              const dataOriginal = d.date || d.data_compra;
              row.push(
                dataOriginal
                  ? new Date(dataOriginal).toLocaleDateString('pt-BR', { timeZone: 'UTC' })
                  : 'N/A'
              );
              break;
            }
            case 'data_compra': {
              // Data de compra específica; se não houver, tenta usar date
              const dataCompraOriginal = d.data_compra || d.date;
              row.push(
                dataCompraOriginal
                  ? new Date(dataCompraOriginal).toLocaleDateString('pt-BR', { timeZone: 'UTC' })
                  : 'N/A'
              );
              break;
            }
      ...
      // --- Fim do mapeamento das despesas
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-2 pt-2">
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onCheckedChange={() => handleColumnChange('data')}
              />
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                id="col_data_compra"
                checked={colunas.data_compra}
                onCheckedChange={() => handleColumnChange('data_compra')}
              />
              <Label htmlFor="col_data_compra" className="font-normal">
                Data da compra
              </Label>
            </div>
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                id="col_descricao"
                checked={colunas.descricao}
                onCheckedChange={() => handleColumnChange('descricao')}
              />
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                id="col_parcela"
                checked={colunas.parcela}
                onCheckedChange={() => handleColumnChange('parcela')}
              />
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                id="col_categoria"
                checked={colunas.categoria}
                onCheckedChange={() => handleColumnChange('categoria')}
              />
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                id="col_valor"
                checked={colunas.valor}
                onCheckedChange={() => handleColumnChange('valor')}
              />
